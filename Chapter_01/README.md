...
实验结论：在两次编译运行的过程中，代码没有做任何改变，仅仅依靠在g++编译时通过参数-D来控制宏的定义，却得到了两个不同的结果，由此证明预处理指令#ifdef发挥作用，在编译器运行前就处理了代码。


##拓展：在代码执行完成后我们可以继续在编译器中输入*strings*和*strings | grep*来进一步查看我们最终可执行文件是哪些
...
strings作用:把g++编译出来的乱码中的可读字符串文本筛选出来

|管道符作用：把前一条命令的输出结果直接作为下一条命令的输出

grep作用:文本检索过滤器，在海量文本中只保留有关键词的行。
